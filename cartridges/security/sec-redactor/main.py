...
PATTERNS = {
    ...
    "Generic_Token": r"(bearer|token|key)[=: ]+[A-Za-z0-9_\-\.]{20,}", # Token genérico
...
COMPILED_PATTERNS = {name: re.compile(regex, re.IGNORECASE) for name, regex in PATTERNS.items()}
...
def redact_text(text: str) -> dict:
    ...
    for name, pattern in COMPILED_PATTERNS.items():
        ...
        if found:
            matches.extend([f"{name}: {m[:4]}..." for m in found]) # Log parcial seguro
            count += len(found)
