...
async def process(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    print(f"[bold yellow]🚑 Git Healer acionado! Tentando corrigir erro no comando '{failed_command}'...[/bold yellow]")

    history = []
    current_error = error_output
    
    for attempt in range(1, max_retries + 1):
        print(f"[bold cyan]🔄 Tentativa de Cura {attempt}/{max_retries}...[/bold cyan]")
        ...
        try:
            ...
            prescription = prescription.replace("```bash", "").replace("```", "").strip()
            
            commands = [cmd.strip() for cmd in prescription.split('\n') if cmd.strip()]
        ...
        for cmd in commands:
            ...
            if not exec_res.get("success"):
                ...
                print(f"[red]❌ Falha na cura ao executar '{cmd}': {current_error}[/red]")
                break
            else:
                print(f"[green]✅ '{cmd}' executado com sucesso.[/green]")
        
        history.append({"attempt": attempt, "commands": commands, "success": batch_success, "error": current_error})

        if batch_success:
            # Se a cura funcionou (todos os comandos de correção rodaram),
            # tentamos rodar o comando ORIGINAL que havia falhado.
            print(f"[bold green]✨ Cura aplicada! Re-tentando comando original: '{failed_command}'...[/bold green]")
            ...
            if retry_orig.get("success"):
                ...
                print(f"[red]🤕 O comando original ainda falha. Tentando nova estratégia...[/red]")
