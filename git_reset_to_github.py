...
class GitResetAutomator:
    ...
    def check_git_repository(self) -> bool:
        ...
        return_code, stdout, stderr = self.run_command(["git", "rev-parse", "--git-dir"])
    ...
    def get_current_branch(self) -> Optional[str]:
        ...
        if return_code == 0 and stdout.strip():
            ...
            return_code, stdout, stderr = self.run_command(["git", "rev-parse", "--abbrev-ref", "HEAD"])
            if return_code == 0 and stdout.strip():
                self.original_branch = stdout.strip()
                self.log(f"✓ Branch atual: {self.original_branch}")
                return self.original_branch
    ...
    def get_all_branches(self) -> Tuple[List[str], List[str]]:
        ...
        if self.verbose:
            self.log(f"Branches locais: {', '.join(local_branches) if local_branches else 'Nenhuma'}")
            self.log(f"Branches remotas: {', '.join(remote_branches) if remote_branches else 'Nenhuma'}")
    ...
    def check_remote_exists(self, remote_name: str = "origin") -> bool:
        ...
        if return_code == 0:
            ...
            if remote_name in remotes:
                ...
                self.log(f"✗ Remote '{remote_name}' não encontrado. Remotes disponíveis: {', '.join(remotes)}", "ERROR")
                return False
        else:
    ...
    def get_remote_url(self, remote_name: str = "origin") -> Optional[str]:
        ...
        return_code, stdout, stderr = self.run_command(["git", "remote", "get-url", remote_name])
        ...
        if return_code == 0:
            ...
            self.log(f"✗ Não foi possível obter a URL do remote '{remote_name}'", "ERROR")
            return None
    ...
    def find_matching_remote_branch(self, local_branch: str) -> Optional[str]:
        ...
        """
        self.log(f"Procurando branch remota correspondente a '{local_branch}'...")
        ...
        for remote_branch in remote_branches:
            for possible in possible_names:
                if remote_branch == possible or remote_branch.endswith(f"/{possible}"):
                    self.log(f"✓ Branch remota correspondente encontrada: {remote_branch}")
                    return remote_branch
        
        self.log(f"✗ Não foi encontrada uma branch remota correspondente para '{local_branch}'", "WARNING")
    ...
    def check_for_uncommitted_changes(self) -> Tuple[bool, str]:
        ...
        if return_code == 0:
            if stdout.strip():
                ...
                self.log(f"✓ Encontradas {len(changes)} alterações não commitadas")
                return True, stdout
            else:
    ...
    def discard_all_changes(self) -> bool:
        ...
        return_code, stdout, stderr = self.run_command(["git", "checkout", "--", "."])
        if return_code != 0:
    ...
    def fetch_remote(self, remote_name: str = "origin") -> bool:
        ...
        return_code, stdout, stderr = self.run_command(["git", "fetch", remote_name])
    ...
    def hard_reset_to_remote(self, remote_branch: str) -> bool:
        ...
        return_code, stdout, stderr = self.run_command(["git", "reset", "--hard", remote_branch])
        
        if return_code == 0:
            self.log(f"✓ Reset --hard para '{remote_branch}' concluído")
            
            # Verifica o commit atual
            return_code, commit_hash, _ = self.run_command(["git", "rev-parse", "--short", "HEAD"])
            if return_code == 0:
                self.log(f"✓ Agora no commit: {commit_hash.strip()}")
            
            return True
        else:
            self.log(f"✗ Erro ao executar reset --hard para '{remote_branch}'", "ERROR")
            return False
    ...
    def cleanup_remaining_files(self) -> bool:
        ...
        return_code, stdout, stderr = self.run_command(["git", "clean", "-fdn"])
        
        if return_code == 0 and stdout.strip():
            files_to_clean = [line.strip() for line in stdout.strip().split('\n') if line.strip()]
    ...
    def verify_sync(self, remote_branch: str) -> bool:
        ...
        # Verifica diferenças
        return_code, stdout, stderr = self.run_command(["git", "diff", remote_branch])
        if return_code == 0:
            if not stdout.strip():
    ...
    def execute_full_reset(self) -> bool:
        ...
        if not self.cleanup_remaining_files():
            self.log("Alguns problemas na limpeza final, mas continuando...", "WARNING")
    ...
    def print_summary(self):
        ...
        # Branch atual
        return_code, branch, _ = self.run_command(["git", "branch", "--show-current"])
        if return_code == 0:
            print(f"📌 Branch atual: {branch.strip()}")
        
        # Último commit
        return_code, commit, _ = self.run_command(["git", "log", "--oneline", "-1"])
        if return_code == 0:
        ...
        if return_code == 0:
            if status.strip():
                print(f"📋 Alterações pendentes: {len(status.strip().split(chr(10)))} arquivo(s)")
            else:
