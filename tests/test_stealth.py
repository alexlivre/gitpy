from tool_stealth import (PRIVATE_CONFIG_FILE, TEMP_DIR_NAME, ensure_gitignore,
                          restore, stash)
...
spec = importlib.util.spec_from_file_location(
    "tool_stealth", os.path.abspath("cartridges/tool/tool-stealth/main.py"))
...
class TestStealthMode(unittest.TestCase):
    ...
    def test_ensure_gitignore(self):
        ...
        with open(os.path.join(self.repo_path, ".gitignore", encoding="utf-8"), "r") as f:
            content = f.read()
        self.assertIn(f"{TEMP_DIR_NAME}/", content)

        # Case 2: Append to existing
        with open(os.path.join(self.repo_path, ".gitignore", encoding="utf-8"), "w") as f:
            f.write("*.log\n")
    ...
    def test_stash_restore_flow(self):
        ...
        self.assertFalse(os.path.exists(
            os.path.join(self.repo_path, "secret.txt")))
        self.assertFalse(os.path.exists(
            os.path.join(self.repo_path, ".secret/key.pem")))
        self.assertTrue(os.path.exists(
            os.path.join(self.repo_path, "public.txt")))

        # Verify temp exists
        temp_path = os.path.join(self.repo_path, TEMP_DIR_NAME)
        self.assertTrue(os.path.exists(os.path.join(temp_path, "secret.txt")))
        self.assertTrue(os.path.exists(
            os.path.join(temp_path, ".secret/key.pem")))
    ...
    def test_restore_conflict(self):
        ...
        with open(os.path.join(self.repo_path, "conflict.txt", encoding="utf-8"), "r") as f:
    ...
    def test_startup_recovery(self):
        ...
        with open(os.path.join(temp_dir, "recover_me.txt", encoding="utf-8"), "w") as f:
            f.write("I survived")
        ...
        self.assertTrue(os.path.exists(
            os.path.join(self.repo_path, "recover_me.txt")))
        with open(os.path.join(self.repo_path, "recover_me.txt", encoding="utf-8"), "r") as f:
