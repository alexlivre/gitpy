...
def setup():
    ...
    if os.path.exists(TEST_DIR):
        ...
        if os.path.exists(TEST_DIR):
            ...
            subprocess.run(
                ["cmd", "/c", f"rd /s /q {TEST_DIR}"], capture_output=True)
...
def test_infra_scanner():
    ...
    # Cenário: Repo Limpo
    cmd = [PYTHON_EXE, os.path.join(
        APP_DIR, "launcher.py"), "main", "--path", TEST_DIR, "--yes"]
...
def test_security_blocklist():
    ...
    # Tentar commitar
    cmd = [PYTHON_EXE, os.path.join(
        APP_DIR, "launcher.py"), "main", "--path", TEST_DIR, "--yes", "--model", "groq"]
    ...
    is_committed = subprocess.run(
        ["git", "ls-files", ".env"], cwd=TEST_DIR, capture_output=True, text=True).stdout.strip()
...
def test_security_redactor():
    ...
    cmd = [PYTHON_EXE, os.path.join(APP_DIR, "launcher.py"), "main", "--path",
           TEST_DIR, "--yes", "--message", "feat: adiciona segredo", "--model", "groq"]
    success, out, err = run_cmd(cmd, APP_DIR)
    ...
    if success:
        ...
        if "[REDACTED:Generic_Token]" in out or "[REDACTED:Generic_Token]" in err:
            ...
            log(
                "⚠️ Redactor warning: Could not verify [REDACTED] tag in output", YELLOW)
    else:
...
def test_e2e_flow():
    ...
    cmd = [PYTHON_EXE, os.path.join(APP_DIR, "launcher.py"), "main", "--path", TEST_DIR,
           "--yes", "--message", "feat: teste e2e automatizado", "--model", "groq"]
    success, out, err = run_cmd(cmd, APP_DIR)
    ...
    if success:
        ...
        # Verificar git log
        log_res = subprocess.run(["git", "log", "-1", "--pretty=%s"],
                                 cwd=TEST_DIR, capture_output=True, text=True).stdout.strip()
