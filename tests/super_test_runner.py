...
def setup():
    ...
    with open(readme_path, "w", encoding="utf-8") as f:
        ...
        f.write(
            "Este repositório é usado para validar todas as funcionalidades do GitPy.\n\n")
...
def test_vibe_vault(repo_dir):
    ...
    with open(large_path, "w", encoding="utf-8") as f:
        for i in range(15000):
            f.write(
                f"// Line {i}: Generated test data for Vibe Vault validation\n")
    git(["add", "-A"], repo_dir)
...
def test_context_hint(repo_dir):
    ...
    # Create a change
    with open(os.path.join(repo_dir, "utils.py", encoding="utf-8"), "w") as f:
        f.write('"""Utility functions for testing."""\n\ndef add(a, b):\n    return a + b\n\ndef multiply(a, b):\n    return a * b\n')
    git(["add", "-A"], repo_dir)

    # Run with context hint - use list to preserve quoted argument
    cmd = GITPY_CMD + ["--yes", "--model", "groq",
                       "-m", "adicionando funções utilitárias"]
    log(f"$ {' '.join(cmd)}", CYAN)
