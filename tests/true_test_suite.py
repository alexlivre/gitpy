...
def setup():
    ...
    stealth_main = os.path.join(
        APP_DIR, "cartridges", "tool", "tool-stealth", "main.py")
    if os.path.exists(stealth_main):
        with open(stealth_main, "r", encoding="utf-8") as f:
            content = f.read()
    ...
    # Create README
    with open(os.path.join(repo_dir, "README.md", encoding="utf-8"), "w") as f:
...
def test_add_file(repo_dir):
    ...
    # verify success
    passed = (rc == 0) and ("Commit realizado" in out) and (
        "Push realizado" in out)
    result("Add File Workflow", passed)

    # Verify file on git
    rc_git, out_git, _ = git(
        ["ls-tree", "-r", "HEAD", "--name-only"], cwd=repo_dir)
...
def test_stealth_mode(repo_dir):
    ...
    with open(private_conf, "w", encoding="utf-8") as f:
